...
public class Store {
	...
	/**
	 * Creates an instance of Store. Initializes the stores name, storeType,
	 * maxStock and the items in the stock.
	 */
	public Store() {
		this.name = storeNames.getName();
		this.storeType = StoreType.values()[random.nextInt(4)];
		this.maxStock = this.storeType.getMinStockBound()
				+ random.nextInt(this.storeType.getMaxStockBound() - this.storeType.getMinStockBound() + 1);
	...
	/**
	 * removes a specified item from the list and prints a string saying it was
	 * purchased.
	 * 
	...
	/**
	 * gets the purchasing coefficient for a specified item depending on the
	 * storeType and itemType.
	 * 
	...
	/**
	 * gets the selling coefficient for a specified item depending on storeType and
	 * itemType.
	 * 
	 * @param item an item selected by the player for purchasing.
